...
from . import warnerstarkbot as bot
...
UPSTREAM_REPO_URL = environ.get("REPO_URL")
...
async def upstream(events):
    ups = event
    check = event.send_id
    if int(check) != int(OWNER_ID):
        return
    lol = await event.reply("`Checking for updates, please wait....`")
    ...
    if conf != "now" and not force_update:
        ...
        await event.reply("**do** `/update now` **to update**")
        return
